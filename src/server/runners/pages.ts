...
function extractMediaUrls(page: HubSpotPage): string[] {
  ...
  while ((match = HREF_MEDIA_RE.exec(jsonStr)) !== null) {
    if (match[1]) urls.add(match[1]);
  }
...
export async function importPages(
  ...
  if (dryRun) {
    ...
    await logToTask(taskId, "info", `Dry run completed. ${skipped} pages previewed, ${failed} failed. Ready for real import.`);
  } else {
    ...
    await logToTask(taskId, "info", `Import completed. ${imported} pages created, ${skipped} skipped, ${failed} failed.`);
  }
}
